...
export const getEndOfDayData = async (
  ...
  try {
    const response = await marketstackApi.get<MarketstackResponse<StockData>>("/eod", {
      params,
    });
    ...
    if (axios.isAxiosError(error)) {
      console.error("Erro na API Marketstack:", error.response?.data || error.message);
    } else {
...
export const getIntradayData = async (
  ...
  try {
    const response = await marketstackApi.get<MarketstackResponse<StockData>>("/intraday", {
      params,
    });
